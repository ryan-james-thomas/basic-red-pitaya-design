...
DAC outputs can be written using `dev.dac[<index>].set(<value>).write()` where `<index>` is either 1 or 2 and `<value>` is the voltage to set. You can set both at once using `dev.dac.set(<value>).write()` where `<value>` is either a single voltage that is written to both DACs or is a two-element list of voltages for each DAC.
